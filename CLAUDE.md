...
Until AdSense approves, Adcash display banners run on `/` and `/wordle/` only: a 160x600 rail per side from 1280px, a 300x100 bar at the bottom below (Adcash offers no 320x50). **Nothing loads before consent**: Adcash is not in the IAB GVL and reads no TCF signal, so `components/AdConsent.tsx` asks itself and `components/AdcashSlots.tsx` injects `aclib.js` only for a stored grant (`lib/ad-consent.ts`, key `kontexto_ad_consent`, 12 months, versioned). Revocation deletes Adcash's storage keys and reloads; leaving an Adcash page with the script loaded is a full reload too. The onboarding snippet sits in `<head>` as inert `type="text/plain"` so Adcash's site verification finds it; it is never fetched or run. **Autotag is never executed**: it rotates pop-under, interstitial, in-page push and video, and in a measured run it loaded an advertiser landing page (with its cookies) on page load without a click. Intrusive formats are what the AdSense review rejects. Display zone ids go into `ADCASH_ZONES` in `frontend/lib/adcash.ts` once the dashboard offers them (only after verification); `null` renders nothing. Changing what the banner says about processing requires bumping `AD_CONSENT_VERSION`. Held by `e2e/ad-consent.spec.ts`; the e2e fixture presets a refusal so the banner does not cover other specs (`adConsent: "unset"` opts out).
