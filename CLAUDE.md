...
- Analytics: `collect/token`, `collect` (pageview, optional `share` marker), `collect/heartbeat` (presence + attention when the tab is visible), `collect/share` (share button pressed), `stats/complete` (client completion histograms), `survey/answer` (attribution survey, one answer per fingerprint), `collect/consent` (ad consent banner: `shown`, `required`, `granted`, `denied`, `regranted`, `revoked`, each once per fingerprint via `analytics_consent_seen`, 45 days; dashboard section „Werbung“).
...
Until AdSense approves, Adcash display banners run on every game page (`isAdcashPath` in `frontend/lib/adcash.ts`: `/`, `/wordle/`, the four solo modes, both queues, and everything under `/duel/`, `/koop/`, `/arena/`, `/wordle/duel/`), never on content, legal, admin or `/live/` pages, whose board is captured into a stream. Formats: a 160x600 rail per side from 1280px, a 300x100 bar at the bottom below (Adcash offers no 320x50). The AdSense allowlist in `lib/adsense.ts` deliberately stays at `/` and `/wordle/`. **Nothing loads before consent**: Adcash is not in the IAB GVL and reads no TCF signal, so `components/AdConsent.tsx` asks itself and `components/AdcashSlots.tsx` injects `aclib.js` only for a stored grant (`lib/ad-consent.ts`, key `kontexto_ad_consent`, 12 months, versioned). A first visit is never blocked; a returning player who has not answered (`isAdConsentRequired`: a round on an earlier day, or `AD_CONSENT_REQUIRED_AFTER_ROUNDS` finished rounds) gets the same two buttons as a dialog that needs a choice, never on the legal pages or `/live/`. A refusal survives a version bump, a grant does not. Revocation deletes Adcash's storage keys and reloads; leaving an Adcash page with the script loaded is a full reload too. The onboarding snippet sits in `<head>` as inert `type="text/plain"` so Adcash's site verification finds it; it is never fetched or run. **Autotag is never executed**: it rotates pop-under, interstitial, in-page push and video, and in a measured run it loaded an advertiser landing page (with its cookies) on page load without a click. Intrusive formats are what the AdSense review rejects. Display zone ids go into `ADCASH_ZONES` in `frontend/lib/adcash.ts` once the dashboard offers them (only after verification); `null` renders nothing. Changing what the banner or the privacy policy says about processing, the page scope included, requires bumping `AD_CONSENT_VERSION` (2 since the scope widened on 2026-09-23). Held by `e2e/ad-consent.spec.ts`; the e2e fixture presets a refusal so the banner does not cover other specs (`adConsent: "unset"` opts out).
