...
class TestRetiredAdConsent:
    """The Adcash consent banner was removed on 2026-09-25; init_db drops its ledger and counters."""

    def test_init_db_drops_the_ledger_and_the_counters(self, db_path):
        async def seed():
            db = await get_db(db_path)
            try:
                await db.execute(
                    "CREATE TABLE analytics_consent_seen (fp_hash TEXT, kind TEXT, ts TIMESTAMP)")
                await db.execute(
                    "INSERT INTO analytics_counters (date, metric, dimension, value) "
                    "VALUES ('2026-09-24', 'ad_consent', 'granted', 3), "
                    "('2026-09-24', 'guesses', 'kontexto', 5)")
                await db.commit()
            finally:
                await db.close()
        ...
        async def inspect():
            ...
            try:
                cur = await db.execute(
                    "SELECT COUNT(*) FROM sqlite_master WHERE name = 'analytics_consent_seen'")
                tables = (await cur.fetchone())[0]
                cur = await db.execute("SELECT metric FROM analytics_counters ORDER BY metric")
