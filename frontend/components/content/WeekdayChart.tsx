...
const byDay = DAYS.map((label, i) => {
  ...
  // The article's weekday table reports the arithmetic mean of each day's
  // solve rate. Keep the line on the same aggregation as the prose; the
  // weekday/weekend comparison above deliberately uses weighted totals.
  const dailyRates = dates
    .map((d) => {
      const dayGuesses = guesses.get(d) ?? 0;
      return dayGuesses ? ((solves.get(d) ?? 0) / dayGuesses) * 1000 : 0;
    });
  const rate = dailyRates.reduce((n, value) => n + value, 0) / Math.max(1, dailyRates.length);
  return { label, avg: g / Math.max(1, dates.length), rate };
});
...
export default function WeekdayChart() {
  ...
  return (
    <figure className="my-6">
      ...
      >
        <title id="wdc-title">Rateversuche und Lösungen je 1.000 Rateversuche nach Wochentag</title>
        <desc id="wdc-desc">
          Balken zeigen die durchschnittlichen Rateversuche je Wochentag, eine Linie darüber die
          durchschnittlichen Tagesquoten der Lösungen je 1.000 Versuche. Von Montag bis Freitag
          liegen die Balken hoch, bei rund
        ...
        <text x={W - PAD.r} y={PAD.t - 5} textAnchor="end" className="fill-muted-foreground text-[8px]">
          Linie: Tagesquote je 1.000
        </text>
      </svg>
      <figcaption className="mt-2 text-center text-xs text-muted-foreground">
        Gegenläufige Tageswerte: Am Wochenende gibt es weniger Rateversuche, während mehr Lösungen je
        1.000 Versuche gezählt werden. Die Linie ist der Mittelwert der täglichen Quoten. Gerechnet
        aus 31 Tagesreihen, jede Zahl serverseitig gezählt.
      </figcaption>
    </figure>
  );
}
