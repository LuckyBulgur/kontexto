    ...
    <div className="divide-y divide-border overflow-hidden rounded-xl border bg-card">
      {items.map((f) => (
        <details key={f.q} className="group">
          <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-4 text-small font-medium text-foreground transition-colors hover:bg-accent [&::-webkit-details-marker]:hidden">
            <span>{f.q}</span>
            <ChevronDown
          ...
          <div className="px-5 pb-4 pt-0 text-small text-muted-foreground">{f.a}</div>
        </details>
      ))}
    </div>
  );
