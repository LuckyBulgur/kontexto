    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8">
        ...
        <header className="mb-7 mt-5 max-w-3xl">
          <h1 className="text-h1 font-bold tracking-tight text-foreground sm:text-display sm:leading-tight">{title}</h1>
        </header>
        <div className="max-w-3xl space-y-7 text-body leading-7 text-muted-foreground [&_h2]:scroll-mt-24 [&_h2]:pt-2 [&_h2]:text-lead [&_h2]:font-semibold [&_h2]:leading-snug [&_h2]:text-foreground [&_a]:text-primary [&_a]:underline [&_a]:underline-offset-2 [&_li]:leading-7 [&_ol]:list-decimal [&_ol]:space-y-2 [&_ol]:pl-5 [&_p]:leading-7 [&_ul]:list-disc [&_ul]:space-y-2 [&_ul]:pl-5">{children}</div>
        <nav aria-label="Weitere rechtliche Informationen" className="mt-10 max-w-3xl border-t border-border pt-6">
          <p className="text-micro font-semibold text-foreground">Weitere rechtliche Informationen</p>
          <ul className="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-small">
