...
interface GuessBarProps { word: string; rank: number; total: number; isNew?: boolean; }
...
export default function GuessBar({ word, rank, total, isNew }: GuessBarProps) {
  ...
  return (
    <div className={`relative flex items-center h-10 rounded-lg overflow-hidden mb-1 transition-all ${isNew ? "animate-slideIn" : ""}`}>
      <div className={`absolute inset-y-0 left-0 ${COLOR_CLASSES[color]} rounded-lg transition-all duration-500`} style={{ width: `${width}%` }} />
