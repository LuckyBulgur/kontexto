...
import { freshHostMessages, hostMessageDurationMs } from "@/lib/host-messages";
 ...
 * pressed. It stays for a reading time that depends on its length, pauses while
 * the pointer or focus is on it, and leaves by itself. Only the card takes
 * pointer events, so the board under the empty part of the strip stays usable.
 *
...
export default function HostMessageBanner({
  ...
  useEffect(() => {
    ...
    remaining.current = hostMessageDurationMs(next.text);
    setQueue(rest);
    setLeaving(false);
    setCurrent(next);
    onShown(next.id);
  ...
  // The reading clock. It runs only while the note is visible and not held,
  // and keeps what is left across a pause instead of starting over.
  useEffect(() => {
    if (!current || leaving || paused || !visible) return;
  ...
  }, [current, leaving, paused, visible]);
