...
export default function GuessSuggestions({ suggestions, onSuggestion }: GuessSuggestionsProps) {
  ...
  return (
    <div className="mt-2 flex flex-wrap items-center gap-2">
      <span className="text-small text-muted-foreground">Meintest du</span>
      {suggestions.map((word) => (
        <button
          key={word}
          type="button"
          onClick={() => onSuggestion(word)}
          className="rounded-full border border-border bg-black/5 px-3 py-1 text-small font-medium text-foreground hover:bg-black/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring dark:bg-white/10 dark:hover:bg-white/20"
        >
          {word}
        </button>
      ))}
